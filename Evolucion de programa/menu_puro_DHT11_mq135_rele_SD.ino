...
File dataFile; // Variable para almacenar el archivo de datos

// Declaración de la función saveData
void saveData();

// Declaración de la función executeAction
void executeAction();
...
void setup() {
  ...
  if (!SD.begin(SD_CS_PIN)) {
    lcd.setCursor(0, 0);
    lcd.print("Error al iniciar la");
    lcd.setCursor(0, 1);
    lcd.print("tarjeta SD!");
    while (true); // Detener la ejecución si hay un error con la tarjeta SD
  }
...
void saveData() {
  ...
  if (dataFile) {
    ...
    lcd.setCursor(0, 0);
    lcd.print("Error al abrir el");
    lcd.setCursor(0, 1);
    lcd.print("archivo de datos!");
  }
